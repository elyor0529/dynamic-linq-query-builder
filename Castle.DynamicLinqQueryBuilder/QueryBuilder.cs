...
namespace Castle.DynamicLinqQueryBuilder
{
    ...
    public static class QueryBuilder
    {
        ...
        public static IQueryable<T> BuildQuery<T>(this IQueryable<T> queryable, FilterRule filterRule, out string parsedQuery, bool useIndexedProperty = false, string indexedPropertyName = null)
        {
            ...
            var whereCallExpression = Expression.Call(typeof(Queryable), "Where", new[] { queryable.ElementType }, queryable.Expression, Expression.Lambda<Func<T, bool>>(expressionTree, pe));
        ...
        private static Expression BuildExpressionTree(ParameterExpression pe, FilterRule rule, bool useIndexedProperty = false, string indexedPropertyName = null)
        {
            ...
            {
                var expressions = rule.Rules.Select(childRule => BuildExpressionTree(pe, childRule, useIndexedProperty, indexedPropertyName))
            ...
            if (type == typeof(DateTime) && ParseDatesAsUtc)
            {
                ...
                if (isCollection)
                {
                    var vals = value.Split(new[] { ",", "[", "]", "\r\n" }, StringSplitOptions.RemoveEmptyEntries)
                            ...
                            .Select(p => DateTime.TryParse(p.Trim(), CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal, out tDate) ? (DateTime?)tDate : null).Select(p => Expression.Constant(p, type));
        ...
        private static Expression In(Type type, string value, Expression propertyExp)
        {
            ...
            if (IsGenericList(propertyExp.Type))
            {
                ...
                if (someValues.Count > 1)
                {
                    ...
                    {
                        exOut = Expression.Or(exOut, Expression.Call(propertyExp, method, Expression.Convert(someValues[counter], genericType)));
            ...
            else
            {
                ...
                if (someValues.Count > 1)
                {
                    if (type == typeof(string))
                    {
                        ...
                        while (counter < someValues.Count)
                        {
                            exOut = Expression.Or(exOut, Expression.Equal(Expression.Call(propertyExp, typeof(string).GetMethod("ToLower", Type.EmptyTypes)), Expression.Convert(someValues[counter], propertyExp.Type)));
                            counter++;
                        }
                    }
                    else
                    {
                        ...
                        while (counter < someValues.Count)
                        {
                            exOut = Expression.Or(exOut,Expression.Equal(propertyExp, Expression.Convert(someValues[counter], propertyExp.Type)));
                            counter++;
                        }
                    }
